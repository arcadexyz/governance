...
export async function main(
    ...
    const tx42 = await nftBoostVault.setTimelock(arcadeCoreVoting.address);
    await tx42.wait();
    ...
    const tx26 = await arcadeCoreVoting.authorize(arcadeGSCCoreVoting.address);
    await tx26.wait();
    const tx27 = await arcadeCoreVoting.setOwner(timelock.address);
    await tx27.wait();

    // ================= Timelock =================
    console.log("Decentralize Timelock...");
    const tx28 = await timelock.deauthorize(DEPLOYER_ADDRESS);
    await tx28.wait();
    const tx29 = await timelock.authorize(arcadeGSCCoreVoting.address);
    await tx29.wait();
    const tx30 = await timelock.setOwner(arcadeCoreVoting.address);
    await tx30.wait();

    // ================= ArcadeGSCCoreVoting =================
    console.log("Setting custom quorum thresholds in ArcadeGSCCoreVoting...");
    // LoanCore
    const tx23 = await arcadeGSCCoreVoting.setCustomQuorum(LOAN_CORE_ADDR, PAUSE, PAUSE_QUORUM);
    await tx23.wait();
    // timelock
    const tx17 = await arcadeGSCCoreVoting.setCustomQuorum(timelock.address, INCREASE_TIME, INCREASE_TIME_QUORUM);
    await tx17.wait();
    ...
    const tx41 = await arcadeTreasury.setThreshold(arcadeToken.address, {
        small: ARCD_SMALL,
        medium: ARCD_MEDIUM,
        large: ARCD_LARGE,
    });
    await tx41.wait();
    const tx43 = await arcadeTreasury.setThreshold(ETH_ADDRESS, {
        small: ETH_SMALL,
        medium: ETH_MEDIUM,
        large: ETH_LARGE,
    });
    await tx43.wait();
    const tx44 = await arcadeTreasury.setThreshold(WETH_ADDRESS, {
        small: WETH_SMALL,
        medium: WETH_MEDIUM,
        large: WETH_LARGE,
    });
    await tx44.wait();
    const tx45 = await arcadeTreasury.setThreshold(USDC_ADDRESS, {
        small: USDC_SMALL,
        medium: USDC_MEDIUM,
        large: USDC_LARGE,
    });
    await tx45.wait();
    const tx46 = await arcadeTreasury.setThreshold(USDT_ADDRESS, {
        small: USDT_SMALL,
        medium: USDT_MEDIUM,
        large: USDT_LARGE,
    });
    await tx46.wait();
    const tx47 = await arcadeTreasury.setThreshold(DAI_ADDRESS, {
        small: DAI_SMALL,
        medium: DAI_MEDIUM,
        large: DAI_LARGE,
    });
    await tx47.wait();
    const tx48 = await arcadeTreasury.setThreshold(WBTC_ADDRESS, {
        small: WBTC_SMALL,
        medium: WBTC_MEDIUM,
        large: WBTC_LARGE,
    });
    await tx48.wait();
    const tx49 = await arcadeTreasury.setThreshold(APE_ADDRESS, {
        small: APE_SMALL,
        medium: APE_MEDIUM,
        large: APE_LARGE,
    });
    ...
    const tx33 = await arcadeTreasury.grantRole(await arcadeTreasury.GSC_CORE_VOTING_ROLE(), FOUNDATION_MULTISIG);
    await tx33.wait();
    const tx34 = await arcadeTreasury.grantRole(await arcadeTreasury.CORE_VOTING_ROLE(), FOUNDATION_MULTISIG);
    await tx34.wait();
    const tx35 = await arcadeTreasury.grantRole(await arcadeTreasury.ADMIN_ROLE(), FOUNDATION_MULTISIG);
    await tx35.wait();
    const tx36 = await arcadeTreasury.renounceRole(await arcadeTreasury.ADMIN_ROLE(), DEPLOYER_ADDRESS);
    await tx36.wait();

    // ================= ReputationBadge =================
    console.log("Setup ReputationBadge manager roles...");
    const tx37 = await reputationBadge.grantRole(await reputationBadge.BADGE_MANAGER_ROLE(), MULTISIG);
    await tx37.wait();
    const tx38 = await reputationBadge.grantRole(await reputationBadge.RESOURCE_MANAGER_ROLE(), MULTISIG);
    await tx38.wait();
    const tx39 = await reputationBadge.grantRole(await reputationBadge.ADMIN_ROLE(), MULTISIG);
    await tx39.wait();
    const tx40 = await reputationBadge.renounceRole(await reputationBadge.ADMIN_ROLE(), DEPLOYER_ADDRESS);
    await tx40.wait();
}
